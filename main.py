...
def update():
    ...
    transaction_df = pd.DataFrame(transaction_list, index=date_range)
    gas_fee_df = pd.DataFrame(gas_fee_list, index=date_range)
    block_df_init = pd.read_csv("block.csv", index_col=0)
    transaction_df_init = pd.read_csv("transactions.csv", index_col=0)
    gas_fee_df_init = pd.read_csv("gas_fees.csv", index_col=0)
